...
export async function analyzeWardrobeImageCore(imageDataUrl: string): Promise<WardrobeAnalysis> {
  ...
  const systemPrompt = [
    ...
    `Return occasions as an array (0-3) from: ${OCCASIONS.join(", ")}.`,
        ...
        `Return materials as an array (1-2) from: ${MATERIALS.join(", ")}. Always give your best guess — materials power season matching and outfit suggestions, and the user can correct them before saving. Combine visible texture cues (sheen, weave, grain, knit stitches) with what this garment type is typically made of (t-shirts and shirts → Cotton; jeans and denim jackets → Denim; tailored blazers and coats → Wool, Polyester or Viscose; flowing dresses and blouses → Viscose, Silk, Linen or Polyester; chunky sweaters → Wool, Cashmere, Merino or Acrylic depending on visible fiber thickness and sheen; sporty/technical pieces → Polyester, Polyamide or Elastane; boots and belts → Leather or Suede; watches, jewelry and metal hardware → Metal, Steel, Gold, Silver or Pearl). "Knit" describes a construction technique, not a fiber — never return it as a material even if the garment is visibly knitted; name the actual fiber instead. A garment, shoe or bag visibly decorated with sparkling stones or sequins (trousers or a top with Swarovski/rhinestones, a crystal-studded clutch, sequinned fabric) MUST list that decoration (Swarovski, Crystals, Rhinestones, Diamonds or Sequins) as one of its materials, even when the base fabric is something else. Return an empty array only if the item is genuinely impossible to assess (e.g. heavily obscured or not a garment).`,
