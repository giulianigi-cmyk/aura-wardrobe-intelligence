...
// a fresh selection made directly in the try-on picker (see the FASE 2
// discussion: both entry points funnel into this one function).
//
// FASHN's tryon-max takes exactly one garment per call, so a full outfit
// is applied as a chain: the avatar photo goes in as model_image for the
// first item, and each result becomes the model_image for the next item,
// in the order the items are passed. Caller controls ordering; a
// reasonable default is base layers first (dress/top+bottom) then
// outerwear, then shoes, so later calls don't have to "see through" a
// coat to place shoes correctly.

import { requireSupabaseAuth } from "@/integrations/supabase/auth-middleware";
import { createServerFn } from "@tanstack/react-start";
import { z } from "zod";
import { runFashnTryOn } from "@/lib/fashn.server";
...
export const tryOnOutfitOnAvatar = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: items, error: itemsErr } = await (supabaseAdmin.from("wardrobe_items" as never) as any)
      .select("id, image_url, category, subcategory")
      .eq("user_id", context.userId)
      .in("id", data.itemIds);
    ...
    const blocked = (items as { category: string | null; subcategory: string | null }[])
      .find((i) => i.category && UNSUPPORTED_CATEGORIES.has(i.category));
    if (blocked) {
      return {
        ok: false as const,
    ...
    const orderedItems = data.itemIds.map((id) => (items as any[]).find((i) => i.id === id)!);
    ...
    if (!data.forceRegenerate) {
      ...
      if (cached?.result_image_path) {
        ...
        if (signed?.signedUrl) {
          return { ok: true as const, imageUrl: signed.signedUrl, cached: true as const };
        }
    ...
    let currentModelImage: string;
    try {
      currentModelImage = await fetchAsDataUrl(supabaseAdmin, "avatar-private", avatar.photo_path);
    } catch (e) {
    ...
    for (const item of orderedItems) {
      ...
      try {
        ...
        return { ok: false as const, error: "generation_failed", message: e instanceof Error ? e.message : "Could not load a garment image." };
      }

      const result = await runFashnTryOn(currentModelImage, garmentImage);
      if (!result.ok) {
        return { ok: false as const, error: "generation_failed", message: result.error };
      }
      currentModelImage = result.imageDataUrl;
    }

    // currentModelImage now holds the final composited result as a data
    // URI — persist it into AURA's own private storage so the person can
    // reopen the result later without depending on FASHN's 60-minute
    // base64 retention window.
    const dataUrlMatch = currentModelImage.match(/^data:([^;]+);base64,(.+)$/);
    if (!dataUrlMatch) {
      return { ok: false as const, error: "generation_failed", message: "FASHN returned an unexpected image format." };
    }
    ...
    if (uploadErr) {
      return { ok: false as const, error: "generation_failed", message: `Result generated but could not be saved: ${uploadErr.message}` };
    }
    ...
    return { ok: true as const, imageUrl: signed?.signedUrl ?? "", cached: false as const };
  });
