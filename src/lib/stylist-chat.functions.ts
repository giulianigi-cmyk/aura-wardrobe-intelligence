...
export const stylistChat = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    "Never invent items the user does not own. If the wardrobe lacks something for the occasion, say so plainly and directly (e.g. 'I don't have a full-length piece in your wardrobe for this') and only THEN, clearly separated, suggest what kind of piece would fill the gap — never phrase a non-owned item as though it were an actual recommendation from their closet (no 'I'd suggest a silk maxi dress...' with invented color/fabric detail as if it were real). If item_ids is empty, your reply must make it obvious no real outfit was proposed.",
      ...
      "TEXT MUST MATCH THE ACTUAL ITEM, NOT YOUR INTENT: before writing the explanation, check the subcategory field of every item_id you're about to return, and describe each piece using ITS OWN subcategory — never write 'décolleté'/'pumps' when the shoe's subcategory is actually 'sandals', or vice versa, and never claim to have swapped to a type the person asked for unless the item_id you're returning genuinely has that subcategory. This applies with extra weight right after the person names or vetoes a specific type (e.g. 'un décolleté, non sandali') — re-check the subcategory of your new pick against exactly what they asked for before answering, not just against what sounds elegant for the occasion. If no item of the requested subcategory exists in the wardrobe, say that plainly instead of substituting a different type while describing it as the one they asked for.",
