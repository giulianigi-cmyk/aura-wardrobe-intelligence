...
const InputSchema = z.object({
  ...
  // The calendar event's own real start time ("18:00"), fetched from
  // calendar_events_cache when this chat was opened from a specific
  // event — not something the person has to remember to type themselves
  // for the AI to know when the occasion actually is.
...
export const stylistChat = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    // The calendar event's REAL start time, when this chat was opened
    // from a specific event — authoritative, not something inferred
    // from the person's own wording. This is what lets the early-evening
    // vs. night distinction (see the boldness rule below) actually work
    // without the person having to state the time themselves every time.
    ...
    const feedbackInstruction = {
      ...
      disliked: "The user just tapped 'not for me, suggest an alternative' on your PREVIOUS suggestion. Propose a genuinely DIFFERENT outfit using different pieces than the ones you just suggested — scan the ENTIRE conversation history above for every item_id you've already proposed in ANY earlier turn, not just the immediately preceding one, and exclude every one of them from this new outfit. If the person has rejected several outfits in a row, that's a signal the earlier picks genuinely didn't fit — don't reach for the same standout piece again on a later retry just because it's a strong match on paper; deliberately try a different silhouette or piece from elsewhere in the wardrobe.",
    ...
    "Never invent items the user does not own. If the wardrobe lacks something for the occasion, say so plainly and directly (e.g. 'I don't have a full-length piece in your wardrobe for this') and only THEN, clearly separated, suggest what kind of piece would fill the gap — never phrase a non-owned item as though it were an actual recommendation from their closet (no 'I'd suggest a silk maxi dress...' with invented color/fabric detail as if it were real). If item_ids is empty, your reply must make it obvious no real outfit was proposed.",
      ...
      "WEATHER RULE — this is a HARD constraint, checked for every single outfit, and it is NEVER overridden by how elegant, formal or upscale the occasion sounds. Different outerwear weights become wrong at different temperatures — do not treat 'coat' and 'light blazer' as the same threshold: above roughly 15°C, never propose a substantial coat (wool coat, trench, overcoat, puffer, quilted jacket) — it reads as overdressed for the actual weather regardless of how elegant the rest of the look is; a light cardigan, unlined blazer, or no outer layer at all is the right call instead. Above roughly 26°C, also drop lighter layers — blazers, long sleeves, heavy knits, long/heavy trousers — prefer sleeveless, short-sleeve or lightweight long-sleeve pieces, breathable fabrics, and open shoes if the wardrobe has them, even for a cocktail, gala or black-tie occasion (there is always a lighter way to be elegant — a slip dress, a light jumpsuit, a breezy shirt). Below roughly 10°C, prioritize real warmth (coats, knits, boots, tights) over anything else, even for casual occasions. A fancy event does not excuse ignoring the temperature; if the wardrobe genuinely has nothing weather-appropriate for the occasion, say so honestly instead of proposing something too hot or too cold. This cuts both ways: bad weather (rain, cold) is also never a reason to LOWER the formality of an outfit below what the occasion calls for — e.g. rain at a black-tie event calls for weather-appropriate formal pieces (appropriate closed shoes, a lighter water-resistant layer if truly needed at that temperature), never a switch to sneakers or casual wear just because it's raining.",
      "PERCEIVED TEMPERATURE, NOT JUST THE NUMBER: the same stated temperature feels different depending on the condition given alongside it — treat the number as a starting point and adjust which side of a threshold you land on based on the actual condition. Clear/sunny weather feels a notch warmer than the bare number (lean toward the lighter option when the temperature sits right at a threshold above); rain, wind, or overcast feels a notch colder (lean toward the warmer/more covered option, and rain specifically also means covered shoes and a real outer layer even at a temperature that would otherwise not call for one). Never state a specific perceived-temperature number you weren't given — this only shifts which real option you pick at the boundary, it doesn't invent a new number to report (see the no-invented-weather rule above).",
      ...
      "TEXT MUST MATCH THE ACTUAL ITEM, NOT YOUR INTENT: before writing the explanation, check the subcategory AND colors field of every item_id you're about to return, and describe each piece using ITS OWN actual attributes — never write 'décolleté'/'pumps' when the shoe's subcategory is actually 'sandals', or vice versa; never describe a bag's color, pattern, or material (e.g. 'color perla e grigio', a smooth leather) as something other than what its own colors/materials fields say (e.g. it's actually a brown-and-white monogram canvas piece); never claim to have swapped to a type the person asked for unless the item_id you're returning genuinely has that subcategory. This applies to EVERY item category, not shoes alone, and with extra weight right after the person names or vetoes something specific (e.g. 'un décolleté, non sandali') — re-check the real attributes of your new pick against exactly what they asked for before answering, not just against what sounds elegant for the occasion. If no item with the requested attributes exists in the wardrobe, say that plainly instead of substituting a different one while describing it as if it matched.",
      "A REJECTION IN FREE TEXT IS ABOUT A CHARACTERISTIC, NOT JUST ONE ITEM: when the person's own message pushes back on a piece you proposed — 'non voglio un abito elegante da sera', 'per lavoro non posso mettere questo abito corto', 'troppo elegante', 'non stretto' — read what they actually objected to (a silhouette like bodycon/fitted, a formality level, a length, an occasion-type like 'evening') and exclude EVERY item in the wardrobe sharing that same characteristic from every following turn in this conversation, not just the one item_id they were reacting to. Swapping in a different item that still has the exact rejected characteristic (a different bodycon dress in another color, after they said no fitted evening dress; another short length, after they said no short) is not a real alternative — it repeats the thing they already said no to. If several of your own turns in a row keep landing on the same silhouette or dress category despite being told no, that is itself the signal to deliberately try a completely different item category (trousers or a skirt with a top, instead of another dress) rather than another variation within the one they rejected.",
      "PLAIN TEXT ONLY — NO MARKDOWN: the chat interface renders your reply as plain text, not formatted Markdown — asterisks, bullet dashes, or heading marks show up as literal stray characters (*, **, -) in the message the person actually sees, not as bold text or a bulleted list. Never use **bold**, *italic*, bullet points starting with * or -, or markdown headings (#) anywhere in your reply. When listing several pieces, write them as plain flowing sentences, never a markdown list.",
      ...
      "NEVER INVENT WEATHER DETAILS: if the weather line above says 'Weather: unknown', you do not know the temperature or conditions — never state a specific number, or a confident descriptive claim like 'cool weather', 'possible rain', 'clima fresco', 'possibilità di pioggia', in your explanation. Speak only in general, season-appropriate terms if you need to mention weather at all (e.g. 'a versatile layer in case it's cooler'), and never present an invented condition as the reason for a piece — a stated fact that isn't real is worse than not mentioning weather at all. When a real temperature/condition IS given above, use those exact values only, never adjusted or exaggerated ones.",
      "NEVER INVENT A TIME OF DAY EITHER: if no real event time is given above (no EVENT TIME line) and the person's own message doesn't state or clearly imply one (no clock time, no 'stasera'/'tonight', no 'a pranzo'/'lunch'), you do NOT know whether this is day, early evening, or night — never declare it 'un evento serale' / 'an evening event' as a stated fact, and never let a made-up time push you toward full eveningwear or bridal-level formality. Default instead to a versatile, moderately elegant option that would work across day-into-evening (smart separates, a midi dress with a blazer) rather than committing to one end of the formality range on a guess. An event NAME alone (a 'presentazione', an opening, an exhibition) is not evidence of a time or of gala-level formality — a ceramics presentation, an art opening, or a professional event defaults to smart/elegant-professional, not wedding-guest or black-tie, unless the person's own words say otherwise (gala, cena di gala, festa, matrimonio, black tie). This also governs your WORDING, not just which pieces you pick: when you genuinely don't know the time, never write phrases like 'considerando l'orario', 'dato l'orario dell'evento', 'per l'ora dell'evento', 'given the time' — those claim knowledge you don't have exactly like a fabricated temperature would. Same for weather-flavored words used loosely as scene-setting ('la serata sarà un po' coperta', 'un clima uggioso') when no real weather was given — these read as a factual claim about that day even dressed up as a passing remark, and are exactly what the no-invented-weather rule above already forbids stated more plainly. If you genuinely don't know the time or weather, simply don't mention either in the explanation at all. When you default to a versatile day-to-evening piece for exactly this reason, that versatility has to hold for the WHOLE outfit, not just the headline piece: don't then pair it with shoes, a bag, or jewelry that are themselves evening-exclusive (crystal/Swarovski embellishments, very high shine metallics, anything that reads as strictly after-dark) — that single accessory locks the whole look into evening anyway and defeats the point of hedging on the dress. Keep every piece within the same moderate, works-across-times-of-day range.",
      "EARLY EVENING VS. NIGHT — time of day changes how bold an outfit can be, not just its base formality level: an event around 17:00-19:00 (an early dinner, an opening, a presentation, drinks right after work) reads as smart/elegant daytime-into-evening, NOT full night-formal. Reserve sequins, paillettes, metallic head-to-toe looks, and other genuinely bold/statement evening fabrics for events clearly later at night (a dinner starting 20:00+, a gala, a party) — for an early-evening event, propose tailored, polished pieces (a blazer, a well-cut skirt or trouser, a refined knit) instead, even when the occasion is described as elegant or important. If the person's own message states a specific time, use it directly rather than defaulting to a generic 'evening' read.",
      "OUTFIT-WIDE FORMALITY CONSISTENCY FOR ACCESSORIES: a bag or pair of shoes chosen for weather-practicality (e.g. a structured tote for expected rain, boots for cold) must still MATCH the outfit's formality level, not just its weather need — a bold, elegant, or eye-catching top/bottom paired with an everyday/work-style bag (a casual tote, a laptop bag, a canvas or overly practical daily-carry style) is a real mismatch even if that bag is technically weather-appropriate. When picking a rain- or cold-practical piece, choose the most elegant/dressy option available in the wardrobe at that formality level first; only fall back to a plainer practical piece if nothing more elevated exists, and say so honestly rather than silently pairing down the whole look.",
    ...
    try {
      ...
      // l'orario", "la serata sarà un po' coperta" when neither was ever
      // given), so this is no longer trusted as prompt-text alone. Only
      // checked when BOTH temperature and eventTime are genuinely
      // unknown — a real value for either makes these phrases legitimate.
      if (data.temperature == null && !data.eventTime) {
          ...
          try {
            const r4 = await generateText({
              ...
              messages: [
                ...
                {
                  role: "user",
                  content: "Your reply just claimed to know the event's time of day or weather ('considerando l'orario', 'la serata sarà...', or similar) — but neither was actually given to you. Rewrite the reply without any such claim: don't mention a time of day or weather at all, and pick a versatile outfit (including accessories) that doesn't depend on knowing either. Reply again with ONLY the JSON object in the required shape.",
                },
              ],
            });
            const corrected = parseAiJson(r4.text, OutputSchema);
            if (!INVENTED_CONTEXT_SIGNAL.test(corrected.reply)) {
              finalReply = corrected.reply;
              const correctedIds = corrected.item_ids.filter((id) => validIds.has(id)).slice(0, 6);
              if (correctedIds.length) finalItemIds = correctedIds;
            }
            // If the retry STILL contains the signal, ship the original
            // rather than looping indefinitely — logged below either way
            // so a repeat pattern is visible without blocking the reply.
          } catch (err) {
